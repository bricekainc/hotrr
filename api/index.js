...
const {
    BOT_TOKEN,
    BOT_USERNAME, // REQUIRED: Set this in your environment (e.g., RedRoomAccessbot)
    WEBAPP_URL,
...
async function safeSendPhoto(userId, item) {
    // We use a URL button instead of callback_data so it stays when forwarded
    const deepLink = `https://t.me/${BOT_USERNAME}?start=${item.id}`;
    const markup = {
        inline_keyboard: [[{ text: `🔓 View & Unlock - ${item.stars} ⭐`, url: deepLink }]]
    };
    ...
    try {
        ...
        await bot.sendMessage(userId, `🖼 **[Preview Available]**\n${item.caption}`, { reply_markup: markup });
    }
}

async function showPaymentOptions(userId, productId) {
    const item = CATALOG.find(i => i.id === productId);
    if (!item) return;

    await bot.sendMessage(userId, `💳 **Select Payment for:**\n${item.caption}`, {
        reply_markup: {
            inline_keyboard: [
                [{ text: `⭐ Stars (${item.stars})`, callback_data: `stars_${productId}` }],
                [{ text: `🇰🇪 M-Pesa (KES ${item.kes})`, callback_data: `mpesa_${productId}` }],
                [{ text: `🌍 Crypto ($${item.usd})`, callback_data: `crypto_${productId}` }]
            ]
        }
    });
}
...
// -------------------- WEB UI --------------------
app.get('/', (req, res) => {
    res.send(`<!DOCTYPE html><html><head><title>H.O.T Red Room</title><style>body{margin:0;padding:0;background:linear-gradient(45deg,#ff0055,#7000ff,#00d4ff);background-size:400% 400%;animation:gradient 15s ease infinite;font-family:'Segoe UI',sans-serif;display:flex;justify-content:center;align-items:center;height:100vh;}@keyframes gradient{0%{background-position:0% 50%;}50%{background-position:100% 50%;}100%{background-position:0% 50%;}}.glass{background:rgba(255,255,255,0.1);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.2);border-radius:30px;padding:50px;text-align:center;box-shadow:0 25px 50px rgba(0,0,0,0.3);width:350px;}h1{color:white;text-transform:uppercase;letter-spacing:5px;}.btn-5d{position:relative;display:inline-block;padding:20px 40px;color:#fff;background:#ff0055;text-decoration:none;font-weight:900;border-radius:15px;text-transform:uppercase;transition:0.2s;box-shadow:0 10px 0 #990033,0 15px 20px rgba(0,0,0,0.4);transform:translateY(-5px);}.btn-5d:active{box-shadow:0 2px 0 #990033,0 5px 10px rgba(0,0,0,0.4);transform:translateY(3px);}</style></head><body><div class="glass"><h1>RED ROOM</h1><p style="color:white;opacity:0.8;">Premium Automated Access</p><br><a href="https://t.me/${BOT_USERNAME}" class="btn-5d">ENTER BOT</a></div></body></html>`);
});
...
bot.on('message', async (msg) => {
    ...
    if (text.startsWith('/start')) {
        // Check if user came from a deep link (e.g., /start v1)
        const parts = text.split(' ');
        if (parts.length > 1 && parts[1].startsWith('v')) {
            await showPaymentOptions(userId, parts[1]);
            return;
        }
    ...
    if (userState[userId]?.awaitingMpesa) {
        ...
        bot.sendMessage(userId, ok ? "✅ **STK Push Sent!** Enter your M-Pesa PIN." : "❌ **STK Push Failed.**");
    }
});
